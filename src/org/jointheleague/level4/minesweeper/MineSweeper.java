...
public class MineSweeper {
	...
	void initializeMines(int firstCellCol, int firstCellRow) {
		...
		while (minesLeft > 0) {
			...
			if (field[firstCellCol][firstCellRow]) {
				field[firstCellCol][firstCellRow] = false;
				minesLeft++;
			}
		}
	...
	Integer getNeighboringMinesCount(int col, int row) {
		...
		return mines.
				map(mines-> 
					IntStream.range(max(0, col-1), min(HEIGHT, col+2)).flatMap(neighborY -> 
						IntStream.range(max(0, row-1), min(WIDTH, row+2)).map(neighborX ->
	...
	private void createAndShowFrame() {
		...
		resetMenuItem.addActionListener(null); // TODO replace null with method
		...
		IntStream.range(0, HEIGHT).forEach(row -> IntStream.range(0, WIDTH).forEach(col -> {
			...
			b.addActionListener(null); // TODO replace null with lambda
										// expression
